...
前几天在某群讨论了一下，设 p = gcd(ra,rb), 然后对于rc的等式方程分子分母都除掉 gcd，此时 ra rb 互质
然后等式依然是成立的，此时的rc要满足为整数，也就是分母的 sqrt(ra/p * rb/p) 要为整数，此时已经互质了
所以分解质因数的结果没有公因子，那么只能两者都满足质因子分解后次幂为 2的整数倍，所以方程的通解必为 k * u^2, k * v^2 的形式
"""
