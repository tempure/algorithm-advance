...
int main() {
    ...
    //第二个和第三个问题特判只有一个元素的情况
    //因为 如果只有一个元素，那么图中的四个点解除限制之后边权都是INF
    if (s == 1) printf("%d\n%d\n", n, n); 
    else {
        int res = dinic();
        printf("%d\n", res); //第二个问题
        ...
        /*
        这里不需要重新建图然后再求一次最大流，Dinic求最大流的本质就是在找增广路，在没有解除限制之前很多增广路
        已经求出了，改变x1和xn的限制，路径上含有这2个点的路径才会继续产生新的增广路，其他的增广路不会受到影响
        所以直接在当前的残留网络继续求最大流加上原来的最大流即可。

        最大流某种程度上只是一个残留网络的“状态”, 解除x1和xn的限制，重新求出的状态就是可以用多次x1和xn的状态对应的最大流。
        加上原来只能用一次x1和xn的状态，就是所有的状态。
        */
        printf("%d\n", res + dinic());
    }
