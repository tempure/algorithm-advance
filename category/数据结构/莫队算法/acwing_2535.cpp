...
/*
概念辨析： 此题的数对(x,y)和(y,x)视为相同的数对 不需要重复计算
...
二次离线莫队:
主要是前缀和的思想，对于每一个位置pos，给出一个询问区间[l,r]
计算出[l,pos-1]中和pos位置的元素配对的元素个数，设为s[pos]
然后求s[]的前缀和即可

由于是莫队，所以要用上一个算出的询问区间的答案来更新下一个询问的区间
根据f[]和g[]的定义，在莫队更新区间的过程中:
1.如果右指针移动，前驱[1,L]和右指针更新的区间的配对数量无法容易求出来更新答案
2.如果左指针移动，前驱[1,R]和左指针更新的区间的配对数量无法容易求出来更新答案
那么就把这些不容易更新的区间以及与之匹配的前驱保存起来，之后再离线计算一次即可。
*/
...
int main() {
    ...
    for (int i = 1; i < m; i++) q[i].res += q[i - 1].res; //前缀和
