...
性质：对于节点数为N的树，从任意节点向上走到根节点，经过的轻边数量不超过logN
证明参考：https://zhuanlan.zhihu.com/p/319713720
又因为一个轻点是一条重链的开头节点，也就是一条重链的top
这样就意味着任意树上一条路径都可以被划分为O(logN)条连续的重链区间
...
作用：
1.单次O(logN)求lca
2.将树上的路径转化为O(logN)段连续区间 然后用线段树等数据结构维护区间操作即可实现：路径修改/路径求和/子树修改/子树求和
