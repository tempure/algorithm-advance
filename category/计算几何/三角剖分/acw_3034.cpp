...
简单多边形指的是多边形边没有相交的线段，凹凸多边形三角剖分都可以求面积

三角剖分 然后求出每个三角形和圆的交集面积 求和即可
...
//求出直线ab和圆的交点，保存在pa和pb中,同时返回圆心到线段ab的距离mind
//mind 用来判断线段ab和圆的位置关系
double get_circle_line_intersecion(pdd a, pdd  b, pdd &pa, pdd &pb) {
    ...
    if (!on_segment(e, a, b)) mind = min(get_dist(r, a), get_dist(r, b)); //e不在ab中间 就返回到ab较小的一个
    if (dcmp(R, mind) <= 0) return mind; //线段ab和圆没有交点 直接提前返回距离mind
