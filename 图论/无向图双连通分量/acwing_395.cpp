...
/*
    ...
    路径相互分离的定义是 两条路径 所有的边都不一样 不能有相同的边
    所以最终要求的图的状态是 任意一条边 都至少在一个环中 等价于e-DCC

    解法：先tarjan缩点 然后缩点后的边 全部都是桥 而且是拓扑的树
...
int main() {
    ...
    for (int i = 0; i < idx; i ++) //枚举每个边即可
        if (is_bridge[i])
            d[id[e[i]]] ++; //给i的出边所在点的连通分量的度数加一 由于是无向边那么到出边的点又会反过来给i加上
